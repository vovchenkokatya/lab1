...
class RationalValueError(ValueError):
    def __init__(self, message="Некоректний тип даних для арифметичної операції"):
        super().__init__(message)
...
class Rational:
    ...
    def _verify_operand(self, other):
    ...
    def __add__(self, other):
        other = self._verify_operand(other)
    ...
    def __mul__(self, other):
        other = self._verify_operand(other)
        return Rational(self._n * other.n, self._d * other.d)

    def __truediv__(self, other):
        other = self._verify_operand(other)
        return Rational(self._n * other.d, self._d * other.n)
...
def run_lab(filenames):
    for fname in filenames:
        ...
        try:
            ...
            print(f"Сума: {rl.total_sum()}")
            print("Елементи (сортована ітерація):")
            for item in rl:
                print(item, end=" ")
            print("\n")
        except FileNotFoundError:
            print(f"Файл {fname} відсутній.\n")

if __name__ == "__main__":

    print("Перевірка RationalValueError:")
    r = Rational(1, 2)
    try:
        result = r + "текст"
    except RationalValueError as e:
        print(f"Успішно спіймано: {e}\n")
